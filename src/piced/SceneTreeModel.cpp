	...
	, m_dataTree(scene)
{
	...
	t->setName("Union");
	t->setType(SceneTree::OperationNode);
	t->addChild();
	t->addChild();
	t->addChild();
	t->child(0)->setName("vec2( sdSphere(    pos-vec3( 0.0,0.25, 0.0), 0.25 ), 46.9 )");
	t->child(0)->setType(SceneTree::PrimitiveNode);
	t->child(1)->setName("vec2( sdBox(       pos-vec3( 1.0,0.25, 0.0), vec3(0.25) ), 3.0 )");
	t->child(1)->setType(SceneTree::PrimitiveNode);
	t->child(2)->setName("vec2( udRoundBox(  pos-vec3( 1.0,0.25, 1.0), vec3(0.15), 0.1 ), 41.0 )");
	t->child(2)->setType(SceneTree::PrimitiveNode);
}
